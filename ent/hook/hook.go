...
// The StoreEventFunc type is an adapter to allow the use of ordinary
// function as StoreEvent mutator.
type StoreEventFunc func(context.Context, *ent.StoreEventMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f StoreEventFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.StoreEventMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.StoreEventMutation", m)
}
